...
effect(pour(X, Y), state(A,B),state(newA,newB)):-  % Pouring from one jug to another
    ...
    NewA is A - Transfer, % The new amount in the source jug
    NewB is B + Transfer. % The new amount in the target jug
...
replace(1, state(_, B), A, state(A, B)). % Replace the first element of the state pair with a new value
replace(2, state(A, _), B, state(A, B)). % Replace the second element of the state pair with a new value

% Define the search algorithm (depth-first search)
solve :- % To solve the problem,
    initial(S), % Get the initial state
    path([S], [], Sol), % Find a path from the initial state to a goal state
